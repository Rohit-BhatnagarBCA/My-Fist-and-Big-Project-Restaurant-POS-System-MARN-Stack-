...
const IconButton = ({ onClick, children, title, active }) => (
  <motion.button
    onClick={onClick}
    title={title}
...
const Header = () => {
  ...
  return (
    <>
      ...
      >
        <img src={logo} className="h-8 w-8 rounded-full" alt="restro logo" />
        <h1
      ...
      <div className="hidden md:flex items-center gap-3 bg-[#242c38] rounded-full px-5 py-2.5 flex-1 max-w-[440px] border border-transparent focus-within:border-[#BD5D31] transition-colors">
        <FaSearch className="text-[#7d8797] shrink-0" size={14} />
        <input
          type="text"
          placeholder="Search"
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        <div className="flex items-center gap-2 sm:gap-3 pl-1 sm:pl-2">
          <FaUserCircle className="text-[#7d8797]" size={34} />
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className="text-[#7d8797] hover:text-[#BD5D31] transition-colors ml-1"
          >
            <IoLogOut size={26} />
          </motion.button>
        </div>
